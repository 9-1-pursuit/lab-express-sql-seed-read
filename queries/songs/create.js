  ...
  if (!obj.album && !obj.time) {
    try {
      const newSong = await database.one(
        "INSERT INTO songs(name, artist, is_favorite, album_id) VALUES ($1, $2, $3, $4) RETURNING *",
        [
            obj.name, 
            obj.artist, 
            obj.is_favorite,
    ...
    if (!obj.album) {
      try {
        const newSong = await database.one(
          "INSERT INTO songs(name, artist, time, is_favorite, album_id) VALUES ($1, $2, $3, $4, $5) RETURNING *",
        [
            obj.name, 
            obj.artist, 
            obj.time, 
      ...
      try {
        const newSong = await database.one(
          "INSERT INTO songs(name, artist, album, is_favorite, album_id) VALUES ($1, $2, $3, $4, $5) RETURNING *",
        [
            obj.name, 
            obj.artist, 
            obj.album, 
